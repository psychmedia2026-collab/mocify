...
export default function RadioClient(){
 ...
 return <div className={styles.radioShell}><div className={styles.radioIntroRow}><header className={styles.radioPageIntro}><p>MOCIFY RADIO</p><h1>{t.title}</h1><span>{t.text}</span></header><form className={`section-local-search ${styles.radioSearch}`} role="search" onSubmit={submitCountrySearch}><span aria-hidden="true">⌕</span><input type="search" value={search} onChange={e=>setSearch(e.target.value)} placeholder={t.title} aria-label={t.title}/>{search&&<button type="button" onClick={()=>setSearch("")} aria-label={t.title}>×</button>}<button type="submit" aria-label={t.title}>→</button></form></div>
