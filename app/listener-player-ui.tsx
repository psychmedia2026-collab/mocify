  ...
  return <section className="m-player" aria-label="Music player">
    ...
    <div className="m-player-inner">
      ...
      <div className="m-player-tools">
        ...
        <label className="m-volume" aria-label={`Volume, ${Math.round(volume * 100)} percent`}>
          <span aria-hidden="true">◖</span>
          <input type="range" min="0" max="1" step="0.01" value={volume} onChange={(event) => setVolume(Number(event.target.value))} aria-label={`Volume, ${Math.round(volume * 100)} percent`} aria-valuetext={`${Math.round(volume * 100)} percent`} disabled={!hasAudioSource}/>
        </label>
        ...
        <div className="m-queue-wrap">
          <button type="button" className={queueOpen ? "m-queue-button active" : "m-queue-button"} aria-label={queueOpen ? "Close queue" : "Open queue"} aria-expanded={queueOpen} onClick={() => setQueueOpen((open) => !open)}>☷</button>
          {queueOpen && <div className="m-queue-popover" role="dialog" aria-label="Playback queue"><b>Queue</b>{queue.map((track) => <button type="button" key={track.id} className={track.id === currentTrack.id ? "current" : ""} onClick={() => { selectTrack(track, queue, true); setQueueOpen(false); }}><span>{track.title}</span><small>{track.artist}</small></button>)}</div>}
        </div>
