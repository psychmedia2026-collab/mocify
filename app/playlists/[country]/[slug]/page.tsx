...
export default function PlaylistDetail(){
 ...
 return <Shell active="playlists"><section className="playlist-detail page-wrap">
  ...
  <header className="playlist-detail-hero"><div className="playlist-detail-art"><span>{country.flag}</span><b>{chartSize?String(chartSize):"♫"}</b></div><div><small>{chartSize?"COUNTRY CHART":"MOCIFY PLAYLIST"} · {country.country}</small><h1>{playlist.title}</h1><p>{playlist.subtitle}</p><span>{tracks.length} tracks</span><div className="playlist-detail-actions"><button className="m-primary" disabled={!playable.length} onClick={()=>playAll(false)}>▶ Play all</button><button className="m-secondary" disabled={playable.length<2} onClick={()=>playAll(true)}>⤨ Shuffle</button><button className="m-secondary" onClick={()=>setSaved(toggleId(SAVED_PLAYLIST_KEY,playlistPath))}>{saved?"♥ Saved":"♡ Save"}</button></div></div></header>
