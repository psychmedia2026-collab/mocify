...
export default function Track(){
  ...
  return <Shell active="explore" player>
    <section className="track-page page-wrap grid gap-10 py-16 lg:grid-cols-[1fr_280px]">
      ...
      <aside><p className="page-kicker">{t.similar}</p><div className="mt-5 space-y-4">{similar.map(r=>{const item=<><div className={`h-14 w-14 shrink-0 rounded-lg ${r.art}`}/><div><b className="text-sm">{r.title}</b><p className="mt-1 text-[10px] text-zinc-400">{r.genre}</p></div></>; return "href" in r&&r.href?<Link href={r.href} key={r.id} className="flex items-center gap-3">{item}</Link>:<button type="button" key={r.id} className="flex w-full items-center gap-3 text-left" onClick={()=>selectTrack(r,similar,true)}>{item}<span className="ml-auto text-xs opacity-60" aria-hidden="true">▶</span></button>})}</div></aside>
    </section>
  </Shell>
}
