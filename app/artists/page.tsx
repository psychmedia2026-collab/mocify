  ...
  return <Shell active="artists">
    <section className="page-wrap py-16">
      ...
      <div className="mt-3 flex flex-col justify-between gap-5 md:flex-row md:items-end">
        ...
        <span className="prototype-search" aria-label="Artist search coming soon">⌕ Search artists <b>SOON</b></span>
      </div>
    </section>
    ...
    <section className="page-wrap pb-16">
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {artists.map(({name,genre,initials,gradientCss,href,profileReady})=>{
          ...
          const content = <>
            <div className="artist-card-art relative grid aspect-square place-items-center overflow-hidden rounded-xl" style={{background:gradientCss}}>
            ...
            <p className="page-kicker mt-4">{genre}</p><h2 className="mt-2 text-xl font-black">{name}</h2><p className="mt-2 text-xs text-zinc-400">{profileReady ? "View profile →" : "Profile coming soon"}</p>
          </>;
          return profileReady
            ? <Link href={href} key={name} className={className}>{content}</Link>
            : <article id={href.split("#")[1]} key={name} className={`${className} artist-card-soon`}>{content}<span className="prototype-badge artist-soon-badge">SOON</span></article>;
        })}
      </div>
    </section>
  </Shell>
