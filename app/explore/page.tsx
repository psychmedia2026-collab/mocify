...
export default function ExplorePage(){
  ...
  const submitExploreSearch=(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const value=new FormData(event.currentTarget).get("explore-q")?.toString().trim()??"";router.push(value?`/explore?q=${encodeURIComponent(value)}`:"/explore")};
  return <Shell active="explore"><div className="page-wrap py-10">
    <SectionHero theme="explore" {...h} href="#trending"/>
    <div className="subpage-search-row"><form className="explore-search" role="search" onSubmit={submitExploreSearch}><span className="explore-search-icon" aria-hidden="true">⌕</span><input type="search" name="explore-q" defaultValue={query} placeholder={dictionary.common.searchPlaceholder} aria-label={dictionary.common.searchMocify}/><button type="submit" aria-label={dictionary.common.search}>→</button></form></div>
    {query&&<section className="search-results" aria-live="polite"><p className="page-kicker">{t.searchTitle}</p><h2 className="mt-2 text-3xl font-black">{totalMatches?`${totalMatches} ${totalMatches===1?t.result:t.results} ${t.for} “${query}”`:`${t.noResults} “${query}”`}</h2>{totalMatches>0&&<div className="search-results-grid">{trackMatches.map(r=><div className="search-result-item search-result-track" key={`track-${r.id}`}><Link className="search-result-track-link" href={("href" in r&&r.href)?r.href:`/explore#${r.id}`}><small>{t.track} · {r.genre}</small><b>{r.title}</b><small>{r.artist}</small></Link><AddToPlaylist trackId={r.id}/></div>)}{artistMatches.map(a=>a.profileReady?<Link className="search-result-item" key={`artist-${a.name}`} href={a.href}><small>{t.artist} · {a.genre}</small><b>{a.name}</b></Link>:<div className="search-result-item search-result-unavailable" key={`artist-${a.name}`} aria-label={`${a.name} ${dictionary.pages.artists.profileSoon}`}><small>{t.artist} · {a.genre}</small><b>{a.name}</b><span>{dictionary.common.comingSoon}</span></div>)}{genreMatches.map(g=><Link className="search-result-item" key={`genre-${g}`} href="/explore#genres"><small>{t.genre}</small><b>{g}</b></Link>)}{playlistMatches.map(p=><Link className="search-result-item" key={`playlist-${p.slug}`} href={`/playlists/int/${p.slug}`}><small>PLAYLIST</small><b>{p.title}</b><small>{p.subtitle}</small></Link>)}{countryMatches.map(p=><Link className="search-result-item" key={`country-${p.code}`} href={`/playlists/${p.code.toLowerCase()}/top40`}><small>COUNTRY PLAYLIST · {p.flag}</small><b>{p.code==="GB"?"UK Top 40":p.code==="US"?"US Top 40":p.country+" Top 40"}</b></Link>)}</div>}</section>}
    <section id="trending" className="scroll-mt-24 py-10"><div className="home-heading"><div><p className="page-kicker">{t.discover}</p><h2>{t.trending}</h2></div><Link href="/artists">{dictionary.nav.artists} →</Link></div><div className="release-grid home-release-grid">{catalog.slice(0,5).map((r,i)=><ReleaseCard key={r.title} r={r} index={i}/>)}</div></section>
