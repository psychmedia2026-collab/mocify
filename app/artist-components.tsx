...
export function ArtistPortalHeader({active=""}:{active?:string}){const{locale}=useLanguage();const t=artistTranslations[locale];return <header className="artist-portal-header"><ArtistLogo/><nav aria-label={t.nav}><Link className={active==="plans"?"active":""} href="/for-artists">{t.plans}</Link><Link className={active==="studio"?"active":""} href="/studio">{t.studio}</Link><Link className={active==="upload"?"active":""} href="/upload">{t.upload}</Link></nav><div className="artist-portal-actions"><LanguageSwitcher/><Link className="artist-back" href="/">{t.back}</Link><Link className="m-secondary compact" href="/for-artists/login">{t.login}</Link><Link className="m-primary compact" href="/for-artists/signup">{t.getStarted}</Link></div></header>;}
