...
export default function ArtistPlayer(){
 ...
 const loadArtist=()=>{const active=getActiveArtist();const name=active?.name??"Andigo";setArtistName(name);void hydratedPublishedReleases().then(p=>{const own:ArtistTrack[]=[...p,...releases].flatMap(r=>{if(r.artist.toLowerCase()!==name.toLowerCase()||!("audioSrc" in r)||typeof r.audioSrc!=="string")return[];return[{id:r.id,title:r.title,artist:r.artist,genre:r.genre,art:r.art,href:"href" in r?r.href:undefined,audioSrc:r.audioSrc}]});setQueue(own);queueRef.current=own;setIndex(0);indexRef.current=0;setCurrentTime(0);setDuration(0);setPlaying(false);if(audioRef.current){audioRef.current.pause();audioRef.current.src=own[0]?.audioSrc??"";if(own[0])audioRef.current.load()}})};
